...
export default async function AppLayout({
  ...
  return (
    <div className="h-screen bg-slate-50 dark:bg-[#0B0F19] flex overflow-hidden">
      {/* ===== SIDEBAR DESKTOP ===== */}
      <aside className="w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hidden md:flex flex-col h-full flex-shrink-0">
        ...
        <div className="p-4 flex-1 flex flex-col gap-1 overflow-y-auto">
          ...
          {/* Plan Info Card */}
          <div className={`mx-1 mb-2 rounded-xl border p-4 ${
            isPro
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-1.5">
                <Zap className={`w-3.5 h-3.5 ${isPro ? "text-emerald-500" : "text-indigo-500"}`} />
                <span className={`text-xs font-bold uppercase tracking-wider ${
                  isPro ? "text-emerald-600 dark:text-emerald-400" : "text-indigo-600 dark:text-indigo-400"
                }`}>
                  Plano {planName}
                </span>
              </div>
              {isPro && (
                <span className="text-[10px] font-bold bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 px-1.5 py-0.5 rounded-full">
                  Ativo
                </span>
              )}
            </div>
            ...
            <ul className="space-y-1.5 mb-3">
              ...
              <li className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
                ...
                Gerador de Textos (IA)
              </li>
              <li className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
                <Check className="w-3 h-3 text-emerald-500 flex-shrink-0" />
                {isPro ? "Histórico ilimitado" : "Histórico limitado"}
              </li>
              {/* Pro-only features */}
              <li className={`flex items-center gap-2 text-xs ${
                isPro ? "text-slate-600 dark:text-slate-300" : "text-slate-400 dark:text-slate-600"
              }`}>
                {isPro
                  ? <Check className="w-3 h-3 text-emerald-500 flex-shrink-0" />
            ...
            {/* CTA upgrade (Free only) */}
            {!isPro && (
              <Link
                href="/precos"
                className="flex items-center justify-center gap-1.5 w-full py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-xs font-bold hover:opacity-90 transition-opacity"
              >
